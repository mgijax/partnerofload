...
#	that does QC checks for the PAR Relationship Load
#
#  Usage:
#
#      partnerOfQC.sh  filename  
#
#      where
...
#      2:  Unexpected error occured running partnerOfQC.py (prompt to view log)
...
#      6) Call partnerOfQC.py to generate the QC report
#
#
#  Notes:  None
#
...
#  ----------  ---  -------------------------------------------------------
#
#  01/26/2023  sc   Initial development
#
###########################################################################
CURRENTDIR=`pwd`
BINDIR=`dirname $0`

CONFIG=`cd ${BINDIR}/..; pwd`/partnerofload.config
USAGE='Usage: partnerOfQC.sh  filename'
...
{ ${PYTHON} ${PARTNEROFLOAD}/bin/partnerOfQC.py ${INPUT_FILE} 2>&1; echo $? > ${TMP_FILE}; } >> ${LOG}
